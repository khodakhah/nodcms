...
class Installer extends Base
{
    ...
    public $next_url;
    public $self_url;
    private DatabaseEnvConfig $databaseEnvConfig;
        ...
        foreach ($steps as $key => $val) {
            if ($key < $step_key) {
                ...
                $this->data['steps'][] = "<div class='col bg-green-jungle'>" .
                    "<a class='text-center d-block pt-3 pb-3 font-white' href='$url'>" . ($key + 1) . ". " . strtoupper(
                        $val
                    ) . "</a>" .
                    "</div>";
                continue;
            ...
            $this->data['steps'][] = "<div class='col bg-grey-cararra'>" .
                "<span class='text-center d-block pt-3 pb-3 font-grey-mint'>" . ($key + 1) . ". " . strtoupper(
                    $val
                ) . "</span>" .
                "</div>";
        }
     ...
     */
    public function complete(): bool|string|RedirectResponse
    {
        if (!$this->hasAccess() || !$this->hasDatabase()) {
            return $this->_response;
        }
        ...
        if ($myform->ispost()) {
            ...
            foreach ($_SESSION['database_connect'] as $key => $value) {
                $this->databaseEnvConfig->{$key} = $value;
            }
