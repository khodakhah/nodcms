...
class Nodcms_general_model extends CoreModel
{
    ...
    public function get_website_info_options($language_id=null)
    {
        $this->db->select('*');
    ...
    public function get_user_by_email_hash_and_active_code($email_hash, $active_code)
    {
        $this->db->select('*');
        $this->db->from('users');
    ...
    public function check_username_exists($text)
    {
        $this->db->select('*');
        $this->db->from('users');
    ...
    public function check_email_exists($text, $active_register=true)
    {
        $this->db->select('*');
        $this->db->from('users');
    ...
    public function update_user_by_email($data, $email)
    {
        $this->db->where('email', $email);
    ...
    public function update_user_login($user_id, $keep_me_time, $user_agent)
    {
    ...
    public function user_set_new_password($user_id, $new_password)
    {
        $this->db->where('user_id', $user_id);
    ...
    public function user_edit_password($user_id, $new_password)
    {
        $this->db->where('user_id', $user_id);
        $this->db->set('password', $new_password);
        $this->db->update('users');
