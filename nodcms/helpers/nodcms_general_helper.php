 ...
 */
define("LANGUAGE_KEYS_PATTERN", '[A-Za-z0-9\s\_\-\.\,\:\;\|\/\!\'\?\&\@\#\€\{\}\(\)\[\]]+');
...
if( ! function_exists('resetJustLanguageTemp')){
    function resetJustLanguageTemp($array){
        $new_content = "<?php\n" .
            "/**\n" .
            " * Made automatically by NodCMS\n" .
            " * Date: ".date("Y.m.d")."\n" .
            " * Time: ".date("H:i")."\n" .
            " * Find from ".count($array)." Files\n" .
            "*/\n" .
            '$lang_temp = array('."\n";
        $new_content .= '    "'.implode('",'."\n".'    "',$array).'"'."\n";
...
if( ! function_exists('resetLanguageTempFile')){
    function resetLanguageTempFile($language_name = NULL)
    {
        $unique_array = array(
            "January",
            "February",
        ...
        resetJustLanguageTemp($unique_array);

        if($language_name != NULL){
            if(!file_exists(APPPATH.'language/'.$language_name))
                die(APPPATH.'language/'.$language_name." isn't exists!");
            $languages = array(APPPATH.'language/'.$language_name);
        }else{
        ...
        foreach ($languages as $item){
            ...
            foreach ($unique_array as $keys) {
                ...
                $my_language_file_content .= '$lang["' . $keys . '"] = "' . $values . '";' . "\n";
            }
            ...
            if(file_exists($my_lang_file)){
                ...
                $myfile = fopen($my_lang_file, "w") or die("Unable to open file!");
...
if( ! function_exists('findNewLangKeys')){
    function findNewLangKeys($CONTROLLER, $overwrite = FALSE, $remove = FALSE)
    {
        $unique_array = array(
            "January",
            "February",
        ...
        if($overwrite){
            ...
            resetJustLanguageTemp($in_my_temp);
        }
