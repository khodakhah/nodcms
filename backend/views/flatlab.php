...
<head>
    ...
    <link rel="stylesheet" href="<?=base_url()?>assets/flatlab/assets/gritter/css/jquery.gritter.css" type="text/css">
...
<!--header start-->
    ...
    <div class="sidebar-toggle-box">
        ...
        <a href="<?=$base_url?>" class="logo"><?=isset($settings["company"])?$settings["company"]:""?></a>
    <div class="nav notify-row" id="top_menu"></div>
    <div class="container" style="padding-top:10px; ">
        <a target="_blank" href="<?=base_url()?>" class="btn btn-link" style="color: #333;text-decoration: none;font-weight: bold"><?=_l("View Website",$this)?></a>
        <div class="btn-group">
            ...
            <ul class="dropdown-menu extended logout">
                ...
                <li><a href="<?=base_url()?>backend.php/cpanel/admin_setting"><i class=" fa fa-cog"></i><?=_l('Settings',$this);?></a></li>
                <li><a href="<?=base_url()?>backend.php/admin/logout"><i class="fa fa-key"></i><?=_l('Log Out',$this);?></a></li>
            </ul>
        </div>
    </div>
    </header>
    ...
    <aside>
        <div id="sidebar"  class="nav-collapse ">
            ...
            <ul class="sidebar-menu" id="nav-accordion">
                <li><a <?=($page == "home")?'class="active"':''?> href="<?=base_url()?>backend.php/"><i class="fa fa-dashboard"></i><span><?=_l("Dashboard",$this)?></span></a></li>
                <li class="sub-menu">
                    ...
                    <ul class="sub">
                        ...
                        <?php foreach($page_list as $data){ ?>
                            <?php if(allowed_page_fields($data["page_type"],"extension",$all_page_type)){ ?>
                                <li <?=(isset($data_type) && isset($relation_id) && $data_type=="page" && $relation_id==$data["page_id"])?'class="active"':''?>><a  href="<?=$base_url?>extensions/page/<?=$data["page_id"]?>"><i class="fa fa-file-text-o"></i> <?=$data["page_name"]?></a></li>
                                <?php } ?>
                                <?php if(allowed_page_fields($data["page_type"],"gallery",$all_page_type)){ ?>
                                <li <?=(isset($data_type) && isset($relation_id) && $data_type=="page" && $relation_id==$data["page_id"])?'class="active"':''?>><a href="<?=$base_url?>gallery/page/<?=$data["page_id"]?>"><i class="fa fa-file-image-o"></i> <?=$data["page_name"]?></a></li>
                                <?php } ?>
                            <?php } ?>
                        <?php } ?>
                    </ul>
                </li>
                <li><a <?=($page == "comment")?'class="active"':''?> href="<?=base_url()?>backend.php/cpanel/comment"><i class="fa fa-comment"></i><span><?=_l("Comments",$this)?></span></a></li>
                <li><a <?=($page == "page")?'class="active"':''?> href="<?=base_url()?>backend.php/cpanel/page"><i class="fa fa-file"></i><span><?=_l("Pages",$this)?></span></a></li>
                <li><a <?=($page == "uploaded_images")?'class="active"':''?> href="<?=base_url()?>backend.php/cpanel/uploaded_images_manager"><i class="fa fa-upload"></i><span><?=_l("Uploaded pictures",$this)?></span></a></li>
                <li><a <?=($page == "user")?'class="active"':''?> href="<?=base_url()?>backend.php/cpanel/user"><i class="fa fa-users"></i><span><?=_l("Members",$this)?></span></a></li>
                <li><a <?=($page == "language")?'class="active"':''?> href="<?=base_url()?>backend.php/cpanel/language"><i class="fa fa-globe"></i><span><?=_l("Languages",$this)?></span></a></li>
                <li><a <?=($page == "menu")?'class="active"':''?> href="<?=base_url()?>backend.php/cpanel/editmenu"><i class="fa fa-sitemap"></i><span><?=_l("Menu manager",$this)?></span></a></li>
                <li><a <?=($page == "setting")?'class="active"':''?> href="<?=base_url()?>backend.php/cpanel/admin_setting"><i class="fa fa-gears"></i><span><?=_l("Settings",$this)?></span></a></li>
...
<script src="<?=base_url()?>assets/flatlab/js/respond.min.js" ></script>
<script src="<?=base_url()?>assets/flatlab/assets/gritter/js/jquery.gritter.js" ></script>

<!--common script for all pages-->
<script src="<?=base_url()?>assets/flatlab/js/common-scripts.js"></script>

<script type="text/javascript" src="<?=base_url()?>assets/flatlab/assets/bootstrap-inputmask/bootstrap-inputmask.min.js"></script>
